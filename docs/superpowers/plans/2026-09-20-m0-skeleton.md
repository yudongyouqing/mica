...
**Goal:** 让 winGhostty 在 Windows 真机上以单窗口跑起 ConPTY + PowerShell,能日常敲命令(M0 验收标准)。

**Architecture:** 三 crate workspace,核心/壳分离(spec §3):`wg-core`(终端状态机 + PTY + 输入编码,纯逻辑可全平台测试)→ `wg-render`(grid→GPU instances,wgpu)→ `wg-app`(Win32 窗口壳,仅 Windows 可运行)。
...
- wgpu surface 路线(wg-app,已核实):`instance.create_surface_unsafe(SurfaceTargetUnsafe::RawHandle { .. })` + raw-window-handle 0.6 的 `Win32WindowHandle`;**不引入 winit**(spec D2,裸 Win32 壳)。
...
- 开发环境分工:macOS 上开发并测试 `wg-core`/`wg-render`(portable-pty 在 mac 走 unix pty 后端);`wg-app` 只能在 Windows 构建/运行,mac 上仅 `cargo clippy` 编译检查(stub main)。
...
- Create: `crates/wg-core/Cargo.toml`、`crates/wg-core/src/lib.rs`
- Create: `crates/wg-render/Cargo.toml`、`crates/wg-render/src/lib.rs`
- Create: `crates/wg-app/Cargo.toml`、`crates/wg-app/src/main.rs`、`crates/wg-app/src/lib.rs`
- Create: `.github/workflows/ci.yml`

**Interfaces:**
- Consumes: 无(起始任务)
- Produces: workspace 内三个可编译 crate;`wg_core::surface`、`wg_core::pty`、`wg_core::input`、`wg_render::{color, atlas, frame}`、`wg_render::pipeline` 模块路径被后续任务填充/引用;CI 矩阵(mac 测 core+render,win 全量)被后续所有任务依赖
...
[workspace]
resolver = "2"
members = ["crates/wg-core", "crates/wg-render", "crates/wg-app"]
...
`crates/wg-app/Cargo.toml`(windows-rs 依赖在 Task 7 按核实结果加入,此处不含):
...
jobs:
  ...
  macos:
    ...
    steps:
      ...
      - uses: Swatinem/rust-cache@v2
      - run: cargo fmt --all -- --check
      - run: cargo clippy -p wg-core -p wg-render --all-targets -- -D warnings
      - run: cargo test -p wg-core -p wg-render
```

- [ ] **Step 3: 本地验证(mac 上)**

Run: `cargo fmt --all && cargo clippy --workspace --all-targets -- -D warnings && cargo test --workspace`
Expected: 三个 crate 全部编译通过;wg-app 在 mac 上编出 stub;测试 0 失败(尚无测试)。

- [ ] **Step 4: Commit**

```bash
git add Cargo.toml Cargo.lock crates/ .github/
git commit -m "chore: workspace scaffold (wg-core/wg-render/wg-app) + dual-platform CI"
```

---

### Task 2: wg-core::surface —— 终端状态机包装

**Files:**
- Create: `crates/wg-core/src/surface.rs`
- Modify: `crates/wg-core/src/lib.rs`
...
`crates/wg-core/src/surface.rs` 先只写测试骨架(实现部分留最小空壳让编译通过但测试红):
...
```bash
git add crates/wg-core/src/
git commit -m "feat(core): terminal surface wrapping alacritty_terminal (feed/resize/grid/event drain)"
```

---

### Task 3: wg-core::pty —— ConPTY 会话

**Files:**
- Create: `crates/wg-core/src/pty.rs`
- Modify: `crates/wg-core/src/lib.rs`(追加 `pub mod pty;`)
...
### Task 4: wg-core::input —— 键位编码(纯逻辑)

**Files:**
- Create: `crates/wg-core/src/input.rs`
- Modify: `crates/wg-core/src/lib.rs`(追加 `pub mod input;`)
...
Run: `cargo test -p wg-core input`
...
### Task 5: wg-render 纯逻辑 —— 颜色解析 / 位图图集 / 网格→instances

**Files:**
- Create: `crates/wg-render/src/color.rs`
- Create: `crates/wg-render/src/atlas.rs`
- Create: `crates/wg-render/src/frame.rs`
- Modify: `crates/wg-render/src/lib.rs`

**Interfaces:**
- Consumes: Task 2 的 `wg_core::surface::{Surface}`(读 `grid()`);alacritty `vte::ansi::{Color, NamedColor, Rgb}`;`grid[Line(i)][Column(j)]` 取 `Cell{c, fg, bg}`
- Produces(Task 6/7 依赖的精确签名):
  - `color::resolve(color: Color, default_fg: Rgb, default_bg: Rgb) -> [u8; 3]`
  - `atlas::CELL_WIDTH: u32 = 8`、`atlas::CELL_HEIGHT: u32 = 16`(必须等于 `wg_core::surface` 的同名常量数值)
...
```bash
git add crates/wg-render/src/
git commit -m "feat(render): color resolve, bitmap atlas, grid->instances (pure CPU)"
```

---

### Task 6: wg-render GPU 管线(wgpu 30)

**Files:**
- Create: `crates/wg-render/src/cells.wgsl`
- Create: `crates/wg-render/src/pipeline.rs`
- Modify: `crates/wg-render/src/lib.rs`(追加 `pub mod pipeline;`)
...
`crates/wg-render/src/cells.wgsl`:
...
`crates/wg-render/src/pipeline.rs`:
...
`crates/wg-render/src/lib.rs` 追加:

```rust
pub mod pipeline;
```

Run: `cargo fmt --all && cargo clippy -p wg-render --all-targets -- -D warnings && cargo build -p wg-render`
...
### Task 7: wg-app —— Win32 窗口壳接线

**Files:**
- Modify: `crates/wg-app/Cargo.toml`(windows-rs 依赖 + pollster + raw-window-handle)
- Modify: `crates/wg-app/src/lib.rs`
- Create: `crates/wg-app/src/app.rs`
...
- Produces: `wg_app::run()` 真实实现(单窗口 + ConPTY PowerShell + wgpu 渲染);mac 上保持 stub(`#[cfg]` 隔离)
...
`crates/wg-app/Cargo.toml` 追加到文件末尾:
...
#[cfg(not(windows))]
pub fn run() {
    eprintln!("wg-app only runs on Windows.");
}
```

`crates/wg-app/src/app.rs`:
...
use wg_core::input::{self, Key, Mods};
use wg_core::pty::{PtyReader, PtySession, default_shell_command};
use wg_core::surface::{CELL_HEIGHT, CELL_WIDTH, ScreenSize, Surface};
use wg_render::frame::build_instances;
use wg_render::pipeline::{Renderer, create_context};
...
struct Terminal {
    ...
    session: PtySession,
    reader: PtyReader,
    ctx: wg_render::pipeline::GpuContext,
    renderer: Renderer,
...
pub fn run() {
    unsafe {
        ...
        let class_name = HSTRING::from("wg_app_class"); // Param<PCWSTR> 的已证实实现
        let title = HSTRING::from("winGhostty");
...
/// 输入约定(见 wg_core::input 文档):Enter/Tab/Esc/Backspace/普通字符走
...
Run(Windows):`cargo fmt --all && cargo clippy -p wg-app --all-targets -- -D warnings && cargo run -p wg-app`
Expected:
...
**Files:**
- Create: `crates/wg-core/tests/powershell.rs`
- Create: `README.md`
...
`crates/wg-core/tests/powershell.rs`:
...
Run(Windows):`cargo test -p wg-core --test powershell`
...
- Windows 10 1809+ / Windows 11,Rust stable:`cargo run -p wg-app`
- 核心与渲染层跨平台:在任意 OS 上 `cargo test -p wg-core -p wg-render`

## 架构

| crate | 职责 |
|---|---|
| `wg-core` | 终端状态机(alacritty_terminal)+ PTY(ConPTY)+ 键位编码,零 GUI 依赖 |
| `wg-render` | wgpu 渲染:字形图集、颜色解析、grid → GPU instances |
| `wg-app` | Win32 窗口壳:消息循环、输入路由、系统接线 |
...
Run(mac):`cargo fmt --all && cargo clippy --workspace --all-targets -- -D warnings && cargo test -p wg-core -p wg-render`
...
```bash
git add crates/wg-core/tests/ README.md
...
2. `cargo run -p wg-app`:窗口出现,暗色标题栏,PowerShell 提示符 + 块状光标可见
...
1. **sRGB 双重编码**(pipeline.rs 的 shader 颜色路径):shader 无 gamma 处理,surface 格式若为 `*Srgb` 会全色偏色。**处方已写入上文 Task 7 的执行者必读警告**(选非 sRGB 格式)。
2. **SGR 7 反色未实现**(frame.rs `build_instances` 忽略 `Flags::INVERSE`,已核 alacritty 在 `Attr::Reverse` 时置位):PowerShell/PSReadLine 选中高亮依赖反色,冒烟时选中会"无视觉反馈"。处方:`cell.flags.contains(Flags::INVERSE)` 时互换 fg/bg(与光标格同路),+1 测试。计划原本把 flags 全忽略定为 M0 边界但漏写了 frame.rs 的边界注释——修 INVERSE 或补注释,二选一。
3. **僵尸进程**(pty.rs Drop):portable-pty 的 kill 是 SIGHUP→250ms→SIGKILL,**SIGKILL 路径不 wait**;子进程 trap HUP 时每关一标签留一僵尸。处方:Drop 里 kill 后补 `let _ = self.child.try_wait();`。

### 🟡 顺手修(小,不阻塞)

4. **锁中毒**(surface.rs `EventProxy::send_event`):持锁调外部闭包,闭包 panic 即毒化锁、该标签永久炸。处方:`lock().unwrap_or_else(|e| e.into_inner())`。
5. **实例缓冲零余量**(pipeline.rs `upload_instances`):单调递增的 cell 数(拖拽放大)每帧重建 GPU 缓冲。处方:扩容取 `max(needed, capacity * 2)`。
...
9. 调色板三处定义(wg-core `default_rgb` / wg-render `BASE16[0]` / `frame::DEFAULT_BG`)——M1 收敛为 wg-core 单一来源(与终审建议 #3 合并)。
